...
if __name__ == '__main__':
    ...
    pickNPlace.move_to_start(Pose(position=Point(x=0.7, y=0.135, z=0.35),orientation=overhead_orientation))
    ...
    board_setup = ['r******r', '**p*****', 'k*******', '', '', '*******K', '***P****', 'R******R']

    piece_positionmap = dict()
    piece_names = []
    
    # This is the intial location where the pieces spawn on the table
    spawnLocation = deepcopy(board_pose)
    spawnLocation.position.x = 0.6
    spawnLocation.position.y = 0.6
    spawnLocation.position.z = 0.8
    ...
    for row, each in enumerate(board_setup):
        for col, piece in enumerate(each):
            ...
            if piece in pieces_xml:
                try:
                    spawn_sdf = rospy.ServiceProxy('/gazebo/spawn_sdf_model', SpawnModel)
                    spawn_sdf("%s%d" % (piece, col), pieces_xml[piece], "/", spawnLocation, "world")
                except rospy.ServiceException, e:
                    rospy.logerr("Spawn SDF service call failed: {0}".format(e))
            ...
            if piece in list_pieces:
                ...
                pickNPlace.pick(Pose(position=Point(x=0.6, y=0.6, z=-0.14), orientation=overhead_orientation))
                # place the chess piece at the defined position
                pickNPlace.place(Pose(position=Point(x=pose.position.x, y=pose.position.y, z = -0.14), orientation=overhead_orientation))
